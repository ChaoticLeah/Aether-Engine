...
const snippetLocation = `../code/codeSnippets/`;
...
export function convert() {
  ...
  for (let i = 0; i < totalCodeFields; i++) {
    ...
    let file = `${snippetLocation}${currentCodeField.getAttribute("script")}`;
    fetch(file)
      .then((req) => req.text())
      .then((js) => {
  ...
  for (let i = 0; i < document.getElementsByClassName("copyCode").length; i++) {
    ...
    let file = `${snippetLocation}${copyButton.getAttribute("script")}`;
    fetch(file)
      .then((req) => req.text())
      .then((js) => {
